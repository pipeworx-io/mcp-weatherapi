...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'WeatherAPI.com');
}
...
async function marine(q: string, days: number | undefined, apiKey: string) {
  ...
  const location = data.location ?? {};
  const forecastday = data.forecast?.forecastday ?? [];
  const marine = forecastday.map((d: any) => ({
    date: d.date,
    maxtemp_c: d.day?.maxtemp_c,
    condition: d.day?.condition?.text,
    hours_sample: (d.hour ?? [])
  ...
  // WeatherAPI's Marine API is a paid add-on. On a plan without it, /marine.json
  // still answers 200 — with an ordinary land forecast and no marine fields at
  // all. That is the dangerous shape: the caller asked for swell and gets back a
  // well-formed object containing an air temperature and "Patchy rain nearby",
  // which reads like an answer. Our trial lapsed to Free on 2026-07-28 and this
  // has been the response ever since. Detect the missing measurements rather
  // than the plan, so this stays correct whichever way the entitlement moves.
  const hasMarineData = marine.some((d: any) =>
    d.hours_sample.some((h: any) => h.sig_ht_mt != null || h.swell_ht_mt != null || h.water_temp_c != null),
  );
  if (!hasMarineData) {
    throw new Error(
      'upstream_down: WeatherAPI returned a forecast with no marine measurements — no wave height, ' +
        'swell or water temperature — which is what it does when the account lacks the paid Marine ' +
        'add-on. Withheld rather than returned, because the response otherwise looks like a valid ' +
        'answer while containing only air temperature. Use `open_meteo_marine` instead: it is keyless ' +
        'and returns hourly wave height and period for any coastal latitude/longitude.',
    );
  }
